...
def test_restore(runner: CliRunner, package: str):
    result = runner.invoke(app, ['install', package, '--using', 'image'])
    print(f'malevich install {package} --using image')

    assert result.exit_code == 0

    module_path, _ = assert_installed_package(package, 'image')
    shutil.rmtree(module_path)
    with pytest.raises(Exception):
        assert_installed_package(package, "image")
    result = runner.invoke(app, ["restore"])
    module_path, stub_path = assert_installed_package(package, 'image')

    wipe_installed_package(module_path, stub_path, package)

def test_remove(runner: CliRunner, package: str):
    result = runner.invoke(app, ['install', package, '--using', 'image'])
    assert result.exit_code == 0
    module_path, _ = assert_installed_package(package, 'image')

    result = runner.invoke(app, ['remove', package])
    assert result.exit_code == 0
    assert not os.path.exists(module_path), 'Stub not removed'
    assert not manf.query(
        'dependencies',
        package
    ), f'{package} still registered in manifest'