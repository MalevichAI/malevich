...
class LocalTask(CoreTask):
    ...
    def prepare(
        ...
        **kwargs
        ...
        raise RuntimeError(
            "Local mode does not support synchronous preparation"
            " use `async_prepare` instead"
        )
    ...
    async def async_prepare(
        ...
        **kwargs
        ...
        for node in self.state.asset_nodes.values():
            ...
            # real = Path(node.real_path)
            # remote = Path(node.core_path)

            # if not real.exists():
            #     raise FileNotFoundError(
            #         f"File {real.absolute()} not found"
            #     )

            # if real.is_dir():
            #     # Iterate over all files deeply
            #     for file in real.rglob('*'):
            #         if file.is_file():
            #            self.runner.storage.obj(
            #                obj_path=remote / file.relative_to(real),
            #                data=file.read_bytes(),
            #                path_from=file.absolute(),
            #            )
            # else:
            self.runner.storage.obj(
    ...
    def run(self, *args, **kwargs) -> None:
        raise RuntimeError(
            "Local mode does not support synchronous execution"
    ...
    def stop(
        ...
        **kwargs
        ...
        raise RuntimeError(
            "Local mode does not support stopping tasks"
        )

    async def async_stop(self, *args, **kwargs) -> None:
    ...
    def results(
        ...
        **kwargs
        ...
        cout(
            message="Task results are being fetched from Core",
